...
from chargrid import Chargrid2D
...
class LaTr_for_finetuning(nn.Module):
  def __init__(self, config, address_to_pre_trained_weights = None):
    ...
    self.Image_encoder = Chargrid2D(3, 197)
  ...
  def forward(self, lang_vect, spatial_vect, quest_vect, img_vect):
    ...
    img_feat = self.Image_encoder(img_vect)
    img_shape = img_feat.shape
    img_feat = img_feat.reshape(img_shape[0],img_shape[1], -1)
